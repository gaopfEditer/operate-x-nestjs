...
import { IPaginateDto, QueryHook, QueryParams, ServiceListQueryParams } from '../types';
...
> {
    ...
    async delete(data: string[], trash?: boolean) {
        let items: E[] = [];
        if ((this.repository as any) instanceof BaseTreeRepository) {
            items = await this.repository.find({
                where: { id: In(data) } as any,
                withDeleted: this.enable_trash ? true : undefined,
                relations: ['parent', 'children'],
            });
            if ((this.repository as BaseTreeRepository<E>).getChildrenResolve() !== TreeChildrenResolve.DELETE) {
