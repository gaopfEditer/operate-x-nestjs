...
export class PostSearchService {
    ...
    async search(text: string) {
        const { hits } = await this.esService.search<PostEntity>({
            index: this.index,
            query: { multi_match: { query: text, fields: ['title', 'body', 'summary'] } },
        });
    ...
    async create(post: PostEntity): Promise<WriteResponseBase> {
        return this.esService.index<PostSearchBody>({
            index: this.index,
            document: pick(instanceToPlain(post), ['id', 'title', 'body', 'author', 'summary']),
        });
    }
    ...
    async update(post: PostEntity) {
        const newBody: PostSearchBody = pick(instanceToPlain(post), [
            'title',
            'body',
            'author',
            'summary',
